...
def test_clean_step_never_reaches_stage_three():
    ...
    screened = screen_step(
        [("get_exchange_rate", RATES)], "rate?", judge_returning("REGION_1")
    )
    called = []
    result = check_calls(
        1,
        screened,
        [ToolCall("get_exchange_rate", {})],
        escalate_fn=lambda calls: (
            called.append(calls) or melon_returning("safe")(calls)
        ),
    )
...
def test_untrusted_content_into_a_sensitive_sink_escalates_and_melon_decides():
    screened = screen_step(
        [("read_email", INBOX)], "summarize inbox", judge_returning("REGION_2")
    )
    blocked = check_calls(
        1,
        screened,
...
def test_inconclusive_melon_is_the_only_path_to_a_human():
    ...
    result = check_calls(
        1,
        screened,
        [ToolCall("send_money", {"amount": 100})],
...
def test_without_stage_three_wired_escalations_fall_back_to_the_user():
    ...
    result = check_calls(
        1, screened, [ToolCall("send_money", {"amount": 100})], escalate_fn=None
    )
...
def test_a_leak_is_blocked_without_paying_for_stage_three():
    ...
    result = check_calls(
        1,
        screened,
        [ToolCall("send_email", {"to": "someone@example.com"})],
...
def test_one_blockable_call_is_not_waved_through_by_safe_ones_beside_it():
    ...
    screened = screen_step(
        [("get_balance", "Balance: $412.19")], "task", judge_returning("REGION_1")
    )
    result = check_calls(
        1,
        screened,
...
def test_trace_records_masked_regions_and_both_sides_of_the_comparison():
    screened = screen_step(
        [("get_exchange_rate", RATES), ("read_email", INBOX)],
        "rate?",
        judge_returning("REGION_1"),
    )
...
def test_trace_round_trips_through_the_logger(tmp_path):
    ...
    screened = screen_step(
        [("read_email", INBOX)], "summarize", judge_returning("REGION_2")
    )
